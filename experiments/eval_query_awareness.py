...
from quwarts.token_counter import GLOBAL_COUNTER, ensure_precise_tokenizer_ready
from quwarts.extractor import OllamaClient
from quwarts.runner import QuWARTSRunner
from quwarts import config as config_module
from quwarts.config import (
    CACHE_DIR,
    DB_DIR,
    PROJECT_ROOT,
    QUERY_DIR,
...
def load_redd_enabled_query_ids() -> Set[str]:
    ...
    if not REDD_TREND_FILE.exists():
        logger.info("ReDD trend file not found; using all category queries")
        return set()
...
def verify_snapshot_integrity(snapshot_db: Path, snapshot_cache: Path) -> None:
    ...
    if not snapshot_db.exists():
        raise RuntimeError(
            ...
            f"Run preprocessing first (experiments/preprocess_player.py)."
        )
    try:
    ...
    if not attr_index_path.exists():
        raise RuntimeError(
            ...
            f"Re-run preprocessing to rebuild it (experiments/preprocess_player.py)."
        )
...
def ensure_snapshot_artifacts(refresh_snapshot: bool = False) -> Tuple[Path, Optional[Path]]:
    ...
    source_db_candidates = [
        latest_run,
        RESULTS_DIR / "player_workload_preprocess" / "checkpoint" / "Player_preprocessed.db",
        DB_DIR / "quwarts.db",
    ]
    ...
    *,
    ...
    if not snapshot_cache.exists():
        raise RuntimeError(
            ...
            f"Re-run preprocessing (experiments/preprocess_player.py) and ensure the "
    ...
    try:
        ...
        all_category_queries = parse_all_category_queries()
        redd_enabled_ids = load_redd_enabled_query_ids()
        if redd_enabled_ids:
            trend_queries = [
                (qid, sql) for qid, sql in all_category_queries
                if qid in redd_enabled_ids
            ]
            logger.info(
                "Running %d queries aligned to uncommented ReDD IDs",
                len(trend_queries),
            )
        else:
            trend_queries = all_category_queries
            logger.info("Running %d category queries", len(trend_queries))
        if not trend_queries:
            raise RuntimeError("No runnable Player queries found in Query/Player/")
