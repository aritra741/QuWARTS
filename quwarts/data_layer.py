"""
Data layer and state management for QuWARTS.

Owns SQLite storage, the metadata registry, and row-to-chunk provenance.
"""
...
from sqlalchemy import (
    ...
    MetaData, DateTime, Index, select, insert, update, text
)
...
class DataLayer:
    ...
    def __init__(self, connection_uri: str = DATABASE_URI):
        ...
        if is_sqlite:
            db_path = Path(connection_uri.replace("sqlite:///", "", 1))
    ...
    def _define_core_tables(self):
        ...
        # Cell_Provenance: per-cell source (row × column → chunk + document).
    ...
    def insert_chunks(self, chunks: List[TextChunk]) -> int:
        """Insert chunks, skipping existing (doc_id, chunk_index) pairs."""
