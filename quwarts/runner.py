"""
Pipeline orchestration and CLI for QuWARTS.
"""
...
from .schema import NUMERIC_SQL_TYPES, semantic_to_sql_type
...
class QuWARTSRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            db_uri: Optional SQLAlchemy database URI (SQLite by default)
            cache_dir: Optional cache directory (defaults to CACHE_DIR / dataset)
            postgres_uri: Deprecated alias for db_uri
        """
        ...
        uri = db_uri or postgres_uri
        self.data_layer = DataLayer(uri) if uri else DataLayer()
        ...
        for table_name, table_info in lattice.tables.items():
            try:
                ...
                if entity_col:
                    ...
                    # discovered for it.  For a ~100-document table this is ~200
...
def main():
    ...
    parser.add_argument("--preprocess", action="store_true", help="Run offline table synthesis")
    parser.add_argument("--query", help="Execute a SQL query")
    parser.add_argument("--workload", help="Path to the reference workload directory")
    parser.add_argument("--db-uri", dest="db_uri", help="SQLAlchemy database URI (default: local SQLite)")
    parser.add_argument("--stats", action="store_true", help="Show statistics")
    parser.add_argument("--clear-cache", action="store_true", help="Clear cache")
    
    args = parser.parse_args()
    
    runner = QuWARTSRunner(args.dataset, db_uri=args.db_uri)
